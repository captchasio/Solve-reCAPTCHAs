...
background.onMessage.addListener(function (msg) {
    ...
    if (msg.action == "solve") {
        ...
        if (msg.error === undefined) {
			...
			changeSolverButtonState(button, "error", chrome.i18n.getMessage("error"));
            tryAgain(button);
        }
    }
...
function doActionsOnSuccess(msg) {
    ...
    Config.getAll().then(config => {
        ...
        if (callback) {
            ...
            textarea.id = 'solverecaptchas-callback-trigger';
        ...
        if (config.autoSubmitForms === true) {
            let timeout = parseInt(config.submitFormsDelay) * 300;
...
function executeAutoSubmitRule(rules) {
    for (let i = 0; i < rules.length; i++) {
        ...
        if (regExp.test(location.href)) {
            let textarea = document.createElement('textarea');
            textarea.id = 'solverecaptchas-autosubmit-code';
...
function createSolverButton(captchaType, widgetId) {
    let button = $(`
        <div class="captcha-solver" data-state="ready" data-captcha-type="${captchaType}" data-widget-id="${widgetId}">
            ...
            <div class="captcha-solver-info">${chrome.i18n.getMessage("solveWithSolveReCaptchas")}</div>
        </div>
    `);
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
    if (request.command == "getContextMenuEl") {
        ...
        if (request.element == 'input') {
            $('.solverecaptchas-toast .close').click();
...
function showToast(message) {
    if (!$('body > .solverecaptchas-toast-container').length) {
        $('body').append(`<div class="solverecaptchas-toast-container"></div>`);
    }

    let toastEl = $(`
        <div class="solverecaptchas-toast">
            <img src="${chrome.runtime.getURL("assets/images/logo.svg")}" class="solverecaptchas-toast-logo">
            <button type="button" class="close">&times;</button>
            <div class="solverecaptchas-toast-message">${message}</div>
        </div>
    `).appendTo('.solverecaptchas-toast-container');
